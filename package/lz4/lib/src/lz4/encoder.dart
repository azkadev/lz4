...
import 'options.dart';
import 'stubs/compress_filter.dart' if (dart.library.io) 'ffi/compress_filter.dart';
...
class Lz4Encoder extends CodecConverter {
  ...
  /// [ArgumentError].
  Lz4Encoder({this.level = Lz4Option.defaultLevel, this.fastAcceleration = false, this.contentChecksum = false, this.blockChecksum = false, this.blockLinked = true, this.blockSize = Lz4Option.defaultBlockSize, this.optimizeForDecompression = false, this.inputBufferLength = lz4EncoderInputBufferLength, this.outputBufferLength = lz4EncoderOutputBufferLength}) {
  ...
  ByteConversionSink startChunkedConversion(Sink<List<int>> sink) {
    ...
    return _Lz4EncoderSink._(byteSink, level, fastAcceleration, contentChecksum, blockChecksum, blockLinked, blockSize, optimizeForDecompression, inputBufferLength, outputBufferLength);
  }
}

/// Lz4 encoding sink internal implementation.
class _Lz4EncoderSink extends CodecSink {
  _Lz4EncoderSink._(ByteConversionSink sink, int level, bool fastAcceleration, bool contentChecksum, bool blockChecksum, bool blockLinked, int blockSize, bool optimizeForDecompression, int inputBufferLength, int outputBufferLength) : super(sink, _makeLz4CompressFilter(level, fastAcceleration, contentChecksum, blockChecksum, blockLinked, blockSize, optimizeForDecompression, inputBufferLength, outputBufferLength));
}
